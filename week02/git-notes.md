...
- A **branch** is created to allow multiple developers to work on different features concurrently without interfering with each other. Main branches usually called `main`.

## Staging Area

The **staging area** is the intermediate area where the files are kept before they are committed. Here, you can review all the modifications you made and specify which modifications to include in the upcoming commit using `git add` command. The staging area allows you to select which files to commit, even if you've made modifications to several files; for example, if five files were changed, you could commit the modifications to three of them in the next commit and leave the others uncommitted for later. In short, the staging area is your way of deciding to create the next snapshot of the repository using only the changes that were made to specific files.
